...
const AGE_ORDER: AgeBand[] = ["under3", "3to5", "6to12", "13to17", "18to22"];

/** Approximate numeric midpoint of each age band, for range checks. */
const AGE_MIN: Record<AgeBand, number> = {
  under3: 0,
  "3to5": 3,
  "6to12": 6,
  "13to17": 13,
...
const AGE_MAX: Record<AgeBand, number> = {
  under3: 2,
  "3to5": 5,
  "6to12": 12,
  "13to17": 17,
...
const INCOME_MIDPOINT: Record<Exclude<IncomeBand, "preferNotToSay">, number> = {
  under2000: 1000,
  "2000to4000": 3000,
  "4000to6000": 5000,
  "6000to8000": 7000,
...
const NEUTRAL: RuleOutcome = outcome("neutral", 0, false, "");
...
export function hasAnyCondition(opts: {
  ...
  passReason: string;
  failReason: string;
  suspectedCountsAsMaybe?: boolean;
}): Rule {
  ...
  return {
    ...
    evaluate: (a: QuizAnswers): RuleOutcome => {
      ...
      if (match) return outcome("pass", weight, required, opts.passReason);
...
export function alwaysPasses(opts: {
  ...
  return {
    ...
    description: opts.description,
    citation: opts.citation,
    evaluate: (): RuleOutcome => outcome("pass", opts.weight ?? 1, false, opts.reason),
  };
}
