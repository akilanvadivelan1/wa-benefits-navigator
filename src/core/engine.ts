...
import { ALL_PROGRAMS, PROGRAMS_BY_ID } from "./programs/index.ts";
 ...
 */
function evaluateProgram(program: Program, answers: QuizAnswers): Evaluation {
  ...
  for (const rule of program.rules) {
    ...
    if (outcome.required) totalRequired += 1;

    switch (outcome.effect) {
      case "pass":
        score += outcome.weight;
        if (outcome.required) passedRequired += 1;
        if (outcome.reason) {
  ...
  const matchesRequestedHelp =
    answers.helpTypes.length > 0 &&
    program.addresses.some((h) => answers.helpTypes.includes(h));
  if (matchesRequestedHelp) score += 2;
 ...
 */
function orderByDependencies(evaluations: Evaluation[]): Evaluation[] {
  ...
  const depthCache = new Map<ProgramId, number>();
  function depth(id: ProgramId, seen: Set<ProgramId>): number {
  ...
  return [...evaluations].sort((a, b) => {
    ...
    const tier = (e: Evaluation) =>
      e.confidence === "likelyEligible" ? 0 : e.confidence === "noBarriers" ? 1 : 2;
    const ta = tier(a);
    const tb = tier(b);
...
function whyThisOrder(program: Program, includedIds: Set<ProgramId>): string | undefined {
...
export function matchPrograms(answers: QuizAnswers): MatchResult {
  ...
  const recommended: ProgramResult[] = ordered.map((e, i) => ({
    program: e.program,
  ...
  const notLikely: ProgramResult[] = notLikelyEvals.map((e) => ({
    program: e.program,
    confidence: e.confidence,
    score: e.score,
    matchesRequestedHelp: e.matchesRequestedHelp,
